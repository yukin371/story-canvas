...
- `protocol/prompt_packs.py`: builtin prompt packs、项目自定义 pack 加载与 pack summary / resolution；同时负责把前端回传的用户模板保存到 `prompts/illustration-packs/`、把 builtin/default pack 导出到项目作用域，并对可选 `lexicon.subjectPhrases/detailPhrases/modePhrases/commercialPhrases/negativePhrases` 做最小规范化；未命中专用模板时，按同类 use-case fallback 维持 `cover-concept -> cover-poster -> promo`、`product -> prop-relic -> promo -> character` 这类兼容链路
