...
def _parse_labeled_fields(section_text: str) -> dict[str, str]:
    ...
    for raw_line in section_text.splitlines():
        ...
        if matched:
            ...
            initial_value = matched.group(2).strip()
            chunks = [initial_value] if initial_value else []
            continue
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Validate pull request body against repository governance template.")
    parser.add_argument("--body-file", help="Read PR body markdown from a file.")
    parser.add_argument("--event-path", help="Read pull_request.body from a GitHub event payload JSON file.")
