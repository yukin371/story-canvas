...
从语料提升 overlay 时，不要只抽“题材应该发生什么”。至少分清六类信号：

1. `主消费点`: 单章持续卖的是压制反打、关系增量、规则求生、证据推进、项目落地还是身份异常
2. `场景发动机`: 触发器、主角应对、阻力来源、可感知结果
3. `节奏曲线`: 开局拉力、中段抛接、章尾余压分别靠什么维持
4. `对白策略`: 直冲、互怼、试探、遮掩、调情、审问或黑色幽默
5. `意象与触感`: 高频物件、地点、声响、身体反馈和场景残留
6. `风格禁区`: 哪些表达会把本题材写成方案文档、通用爽文、翻译腔或检测器补丁

使用 overlay 起稿时，先把上述信号压成 `叙述距离 / 行文速度 / 句子质地 / 对白压力 / 意象库 / 禁用语域`，再进入正文。

如果某个 overlay 只有“可直接转写为提示词”而缺少对白、意象或句法倾向：

1. 可以作为结构提示使用
2. 不得宣称已具备完整文风迁移能力
3. 写作时必须回退 `writing-universal.md` 的“语料信号转散文”规则补足风格执行
