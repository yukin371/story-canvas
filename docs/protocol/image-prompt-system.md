...
当前 CLI 与本地 UI API 复用同一条生图主链：

- CLI 入口走 `command_illustration_generate()` -> `run_illustration_generation()`
- 本地 UI 入口走 `/api/illustration/generate`，最终也调用 `run_illustration_generation()`
- prompt pack 解析、prompt snapshot 展开、provider request 组装、资产落盘与 `illustrations.yaml` 回录共用同一套命令实现
...
sequenceDiagram
    ...
    participant API as Local UI API
    participant CMD as commands/illustration.py
    participant PP as protocol/prompt_packs.py
    participant SVC as services/illustration_prompting.py
    participant PROV as providers/image/openai_http.py
    participant FS as Project Files

    U->>UI: 提交生图参数
    alt CLI
        UI->>CMD: command_illustration_generate(args)
    else UI 工作台
        UI->>API: POST /api/illustration/generate
        API->>CMD: run_illustration_generation(args)
    end

    CMD->>FS: ensure_project_root + load_project_state
    CMD->>PP: resolve_prompt_pack(root, illustrations_state, prompt_pack)
    PP-->>CMD: pack + packRef

    CMD->>SVC: build_entity/chapter/freeform_illustration_payload(...)
    SVC->>SVC: 选 template / modifier / policy / lexicon
    SVC->>SVC: 生成 promptSnapshot + policySnapshot + promptText
    SVC-->>CMD: payload

    CMD->>CMD: _validate_generation_payload
    CMD->>CMD: _build_provider_request
    CMD->>CMD: _resolve_output_path

    alt text-to-image
        CMD->>PROV: build_generation_request(...)
    else image-to-image / inpaint
        CMD->>PROV: build_edit_request(...)
    end

    PROV->>PROV: 组装 Responses API + image_generation 请求
    PROV->>PROV: generate_image()
    PROV->>PROV: 解析 SSE 响应，提取 response id / revised prompt / artifacts
    PROV-->>CMD: provider result

    CMD->>PROV: materialize_artifacts(result)
    PROV-->>CMD: 图片字节 + 扩展名 + revisedPrompt

    loop 每张返回图片
        CMD->>FS: 写入 assets/illustrations/... 或 tmp/illustrations/...
    end

    CMD->>CMD: _build_generated_entry(...)
    CMD->>FS: save_state() -> illustrations.yaml
    CMD-->>API: illustration entry
    API-->>UI: JSON response
    UI-->>U: 展示结果并刷新 summary
```

实现补充说明：

- `--dry-run` 与正式生成复用相同的前半链路，但会在 provider 实际请求之前停止，只返回 `payload`、`promptSnapshot`、`policySnapshot`、`providerRequest` 与预测输出路径
- `negativePrompt` 当前会并入最终 prompt 文本，作为 `avoid: ...` 子句保留在快照和 provider request 中，而不是作为 OpenAI 独立字段下发
- 资产路径按 target type + use case 自动分流：`chapter` -> `assets/illustrations/chapters/<chapter-id>/<use-case>/`，`entity` -> `assets/illustrations/entities/<entity-id>/<use-case>/`，`temporary` -> `tmp/illustrations/staging/<temp-label>/<use-case>/`
...
- 资产落盘按 target type + use case 自动整理：
