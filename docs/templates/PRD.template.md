...
> 规则:
> 1. 面向中文创作时，字段内容默认用中文填写
> 2. 英文别名只用于内部键名、程序字段或 adapter 映射
> 3. 不要把英文概念说明直接当成写作提示正文

| 中文字段 | 内部别名 | 内容 |
|------|------|------|
| 主角异常状态 | `abnormalState` | {主角当前最不正常、最能抓人的状态} |
| 核心反差 | `coreContrast` | {主角欲望 / 身份 / 处境 与世界要求之间的错位} |
| 读者承诺 | `readerPromise` | {读者持续追看能获得什么体验} |
| 第一卷承诺 | `firstVolumePromise` | {第一卷最小但具体的交付，不写抽象成长} |
| 场景引擎 | `sceneEngine` | {单章最常见触发器、主角应对方式、章尾续追问题} |
| 连载循环 | `serialLoop` | {这本书如何重复产出章节乐趣} |
| 扩张棋盘 | `expansionBoard` | {前 20 章内，个人异常将接到哪块更大棋盘} |

示例写法：

- 主角异常状态:
  “她明明该是替万人迷师妹挡刀而死的工具人二师姐，却在开局第一章就决定不配合剧本。”
- 第一卷承诺:
  “她跳出旧宗门压迫逻辑，并在新宗门建立第一轮身份与价值感。”
- 场景引擎:
  “旧剧本继续压她 -> 她识破并反套路 -> 保住资源或主动权 -> 关系网进一步偏向她”
...
- 写前要求:
  ...
  - 面向中文正文写作时，prompt 正文、示例、改写要求、审查意见默认使用中文
  - 若存在英文内部字段，只能作为程序键名或 adapter 映射，不应直接替代中文创作指令
